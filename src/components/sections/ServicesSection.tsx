      ...
      <div className="grid w-full grid-cols-[repeat(auto-fit,minmax(min(100%,280px),360px))] justify-center gap-4 sm:gap-5">
        {servicesSection.items.map((service) => {
          ...
          return (
            <article
              key={service.title}
              className="group w-full rounded-[1.4rem] border border-slate-200/90 bg-white px-4 py-4 sm:rounded-[1.55rem] sm:px-6 sm:py-6 shadow-[0_12px_30px_rgba(15,23,42,0.06),0_18px_44px_rgba(15,23,42,0.04)] transition-all duration-300 ease-out hover:-translate-y-2 hover:border-slate-300 hover:shadow-[0_20px_44px_rgba(15,23,42,0.1),0_28px_60px_rgba(15,23,42,0.08)]"
            >
              <div className="flex h-12 w-12 items-center justify-center rounded-[1.15rem] bg-[linear-gradient(180deg,#30394a_0%,#111827_100%)] text-white shadow-[0_10px_20px_rgba(15,23,42,0.2)] sm:h-13 sm:w-13 sm:rounded-2xl">
                <Icon className="text-[1.15rem] sm:text-[1.3rem]" />
              </div>
              <h3 className="mt-4 text-[1.18rem] font-bold tracking-[-0.03em] text-slate-900 sm:mt-6 sm:text-[1.45rem]">
                {service.title}
              </h3>
              <p className="mt-2.5 max-w-none text-[0.92rem] leading-6 text-slate-500 sm:mt-3 sm:max-w-[30ch] sm:text-[0.96rem] sm:leading-7">
                {service.description}
              </p>
            </article>
          )
